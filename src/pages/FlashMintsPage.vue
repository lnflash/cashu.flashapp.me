  ...
  <div class="mints-page">
    ...
    <div class="mints-list">
      ...
      >
        <div class="mint-card-left">
          <div class="mint-dot" :class="{ 'mint-dot--active': mint.url === activeMintUrl }" />
          <div class="mint-info">
    ...
    <q-dialog v-model="showAdd" position="bottom">
      <div class="bottom-sheet">
        ...
        <div class="input-wrap" :class="{ 'input-wrap--ok': urlValid, 'input-wrap--error': addInput && !urlValid }">
          <input
            v-model="addInput"
            class="mint-input"
        ...
        <div v-if="!hasFlashMint" class="quick-add" @click="addInput = FLASH_MINT">
  ...
  setup() {
    ...
    const mintItems = computed(() => {
      return (mintsStore.mints || []).map((m: any) => {
        ...
        const balance = Object.values(balances as Record<string, number>).reduce((s: number, v: number) => s + v, 0)
    ...
    return {
      FLASH_MINT,
      showAdd, addInput, addError, adding,
      activeMintUrl, mintItems, hasFlashMint, urlValid,
      toastMsg,
      mintLabel, mintShortUrl,
      activate, doAddMint, confirmRemove,
    }
  },
})
</script>
...
.back-btn:hover, .add-btn:hover { color: #f5f5f5; }
.add-btn { color: #41ad49; }
...
.mint-card:hover { border-color: rgba(255,255,255,.12); }
.mint-card--active {
  border-color: rgba(65,173,73,.4);
  background: rgba(65,173,73,.05);
}

.mint-card-left { display: flex; align-items: center; gap: 14px; }
.mint-dot {
  width: 10px;
  height: 10px;
  border-radius: 50%;
...
.mint-dot--active { background: #41ad49; }

.mint-info {}
.mint-name { font-size: 15px; font-weight: 600; }
.mint-url { font-size: 12px; color: #666; margin-top: 2px; }

.mint-card-right { display: flex; align-items: center; gap: 12px; }
.mint-balance { font-size: 14px; font-weight: 600; color: #41ad49; }
.mint-remove {
  background: none;
  border: none;
  color: #555;
...
.mint-remove:hover { color: #ff4444; background: rgba(255,68,68,.1); }
...
.input-wrap--ok { border-color: rgba(65,173,73,.5); }
.input-wrap--error { border-color: rgba(239,68,68,.4); }
...
.add-error { font-size: 13px; color: #ef4444; padding-left: 4px; margin-bottom: 12px; }
...
.quick-add {
  ...
  gap: 8px;
  padding: 12px 14px;
  background: rgba(65,173,73,.06);
  border: 1px solid rgba(65,173,73,.2);
  border-radius: 12px;
  cursor: pointer;
  font-size: 14px;
  color: #41ad49;
...
.quick-add:hover { background: rgba(65,173,73,.1); }
.quick-add-bolt { font-size: 18px; }
...
.add-mint-btn:disabled { opacity: .35; cursor: not-allowed; }
.add-mint-btn:not(:disabled):hover { background: #4ec256; }
